...
export function GraphView(): JSX.Element {
  ...
  const topFolders = useMemo(() => {
    ...
    for (const node of payload?.nodes ?? []) set.add(topFolderOf(node.path));
    return [...set].sort();
  }, [payload]);
  ...
  useEffect(() => {
    ...
    cy.batch(() => {
      ...
      cy.add(toElements(filtered, isDark, detected, colorMode, folderColors));
    });
    ...
    cy.nodes().layout(layoutOptions(layout, (payload.nodes ?? []).length)).run();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [payload, layout, hideTypeOnly, collapseBarrels, colorMode, folderColors]);
